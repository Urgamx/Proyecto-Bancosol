...
%>
    ...
    <%if(turnos != null && turnos.size() > 0){%>
        <table class="table-striped">
            ...
            <tr>
                <td><a href="/resp-entidad/detalles-voluntario?id=<%=turno.getIdVoluntario().getId()%>&tienda=<%=tienda.getId()%>"><%=turno.getIdVoluntario().getNombre()%></a></td>
                ...
                <td>
                    <a href="/resp-entidad/detalles-voluntario?id=<%=turno.getIdVoluntario().getId()%>&tienda=<%=tienda.getId()%>" class="btn btn-primary btn-sm">Ver Detalles</a>
                    <%if(turno.getIncidencia() == null){%>
                        <a href="/resp-entidad/registrar-incidencia?id=<%=turno.getId()%>&tienda=<%=tienda.getId()%>" class="btn btn-warning btn-sm">Reportar</a>
                    <%}%>
                </td>
            </tr>
            <%}%>
